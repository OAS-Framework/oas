...
function fixtureSoul(base, runtime = "pi") {
...
test("target composition applies global + group + soul specificity and exclusions", () => {
...
test("manifest-declared layers compose from canonical targeted capability bindings", () => {
  const base = temp(); const repo = join(base, "repo"); mkdirSync(repo);
...
test("pre-contract manifest, config, and discovery spellings are rejected or ignored", () => {
  ...
  assert.throws(() => resolveOasConfig(repo, "dev"), /only accepts "none"/);
});
...
test("explicit layer none excludes inherited integrations and same-scope contradictions error", () => {
  ...
  write(join(repo, "oas-config.yaml"), "layers:\n  knowledge: none\ncapabilities:\n  acme.knowledge:\n    global: true\n");
  assert.throws(() => resolveOasConfig(repo, "dev"), /explicitly disabled/);
});

test("equal-specificity group conflicts and competing fundamental integrations error", () => {
  const base = temp(); const repo = join(base, "repo"); mkdirSync(repo);
  capability(repo, "a", { capability: "acme.a", layer: "knowledge" });
  capability(repo, "b", { capability: "acme.b", layer: "knowledge" });
  write(join(repo, "oas-config.yaml"), `groups:\n  one: [dev]\n  two: [dev]\ncapabilities:\n  acme.a:\n    groups:\n      one: {enabled: true, settings: {mode: one}}\n      two: {enabled: true, settings: {mode: two}}\n`);
  assert.throws(() => resolveOasConfig(repo, "dev"), /ambiguous capability setting/);
  write(join(repo, "oas-config.yaml"), `capabilities:\n  acme.a:\n    global: true\n  acme.b:\n    global: true\n`);
  assert.throws(() => resolveOasConfig(repo, "dev"), /multiple active capabilities/);
});
...
test("CLI activation writes stable global/group/soul bindings without activating acquisition", () => {
  const base = temp(); const repo = join(base, "repo"); mkdirSync(repo);
  ...
  for (const argv of [
    ...
    ["use", "oas.okf", "--group", "reviewers", "--disable", "--dir", repo],
