...
test("oas use in a scope with no config chain names the initialization remedy instead of reporting an installed capability as unacquired", () => {
  ...
  assert.ok(r.stderr.includes("oas-config.yaml"), r.stderr);
  ...
  // The remedy is EXECUTED, not paraphrased: the backticked `oas init` command
  ...
  // argv[0] (`oas` is not installed on the test host), and the one thing the
  // harness needs — an offline package catalog — is passed through the
  // environment, never the command line.
  const printed = r.stderr.match(/`(oas init [^`]+)`/);
  ...
  const asNode = command.replace(/^oas\b/, `${JSON.stringify(process.execPath)} ${JSON.stringify(CLI)}`);
...
/** A member repo shipping an OAS distribution package: manifest at `root`, and
...
function packagePayload(repo, { id, root = "oas-package", template = "config-templates/default/oas-config.yaml" } = {}) {
  write(join(repo, root, "oas-package.json"), JSON.stringify({
...
test("scope discovery: package PAYLOAD config templates are never scopes (default and custom payload roots)", () => {
  ...
  const ws = join(base, "ws");
  write(join(ws, "oas-config.yaml"), "name: ws\nteam:\n  name: t\n");
  // Two member repos in the live shape: oas-package/ manifest + exported template.
  packagePayload(join(ws, "oas-jira"), { id: "oas.jira" });
  packagePayload(join(ws, "oas-linear"), { id: "oas.linear" });
  // A CUSTOM payload root (not the literal config-templates/ under oas-package/):
  // a second package at a non-default path, with a non-default template path.
  packagePayload(join(ws, "oas-multi"), { id: "oas.extra", root: join("dist", "extra-pkg"), template: join("profiles", "reference", "oas-config.yaml") });
  // A repo that is a REAL scope AND ships a payload — the framework-repo shape:
  // its root oas-config.yaml has NO oas-package.json ancestor, so it stays a scope.
  write(join(ws, "framework-like", "oas-config.yaml"), "name: framework-like\n");
  packagePayload(join(ws, "framework-like"), { id: "oas.selfhosted" });
  // An ordinary member scope, unrelated to any payload.
  write(join(ws, "plain-member", "oas-config.yaml"), "name: plain\n");
  ...
  write(join(ws, "oas-jira", "oas-package", "config-templates", "default", "oas-lock.json"), JSON.stringify({ lockfileVersion: 1, capabilities: {} }));
  const migration = discoverMigrationScopes(ws, { teamScope: ws });
  assert.equal(migration.filter((s) => s.includes("oas-package")).length, 0, migration.join("\n"));
});

test("bare oas install at a team boundary: payload templates reconcile cleanly, a genuine member failure still reports", () => {
  const base = temp();
  const ws = join(base, "ws");
  write(join(ws, "oas-config.yaml"), "name: ws\nteam:\n  name: t\n");
  ...
  assert.doesNotMatch(clean.stdout + clean.stderr, /oas\.jira|oas\.linear|oas\.extra/, "template-declared capabilities must never be validated or acquired");
  ...
  write(join(ws, "real-member", "oas-config.yaml"), "name: real\ncapabilities:\n  layers:\n    tasks:\n      capability: ghost.cap\n      from: installed\n");
...
test("doctor judges every artifact against ITS OWN scope's lock: one package id at two scopes with different source spellings is clean in both renderings", () => {
  ...
  const pkgRepo = (dir, id, capabilities, dependencies) => {
    ...
    for (const [rel, cm] of Object.entries(capabilities)) {
      rels.push(rel);
      write(join(dir, "oas-package", rel, "oas.json"), JSON.stringify({ version: "2.0.0", description: "cap", ...cm }, null, 2));
    }
    write(join(dir, "oas-package", "oas-package.json"), JSON.stringify({
      package: id, version: "2.0.0", description: "pkg", compatibility: { oas: ">=0.1.0" },
  ...
  const catalog = (id, selector) => {
    if (id === "x.official") return { url: `file://${officialRoot}`, ref: selector || officialCommit, path: "oas-package" };
    if (id === "x.dev") return { url: `file://${devRoot}`, ref: selector || devCommit, path: "oas-package" };
    return undefined;
  };
  ...
  const lockOf = (dir) => JSON.parse(readFileSync(join(dir, "oas-lock.json"), "utf8"));
  ...
  assert.doesNotMatch(human.stdout, /\.oas-installation\.json/, human.stdout);
  ...
  const provenance = join(art, ".oas-installation.json");
  ...
  write(join(outer, "oas-lock.json"), JSON.stringify(doctored, null, 2));
