...
For feature/agent-relations, the sidebar and Instances roster group by agent cluster: the connected component of the undirected relation graph made from spawn parentage plus sibling links.
...
The desktop side keeps the original pluggable seam: `instanceLinks(instance)` in `packages/desktop/renderer/instance-tree.mjs` is the single extractor, and `clusterInstances(list, { links })` also accepts an injected extractor for tests. The final `siblingInstance` field should be forwarded through the server's `/api/panel` projection in `packages/desktop/server/oas-web.mjs`, which must explicitly allowlist it from roster metadata, then consumed by `instanceLinks(instance)` without changing the connected-component grouping algorithm.
