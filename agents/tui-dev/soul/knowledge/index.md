...
* [Async tab lifecycle cleanup must track fulfillment and key reservation](async-mount-close-race.md) — When a desktop tab closes during async mount, cleanup must wait for settle, fall back to module unmount only after a fulfilled legacy mount, and keep dedup keys reserved until cleanup completes.
