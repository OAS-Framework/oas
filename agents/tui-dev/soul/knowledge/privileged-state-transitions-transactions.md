...
- **Readiness must verify identity, not just liveness.** During same-port races,
  ...
  [server reuse identity probe](server-reuse-identity-probe.md) rule and the
  [privileged workspace add contract](desktop-workspace-add-privileged-contract.md).
- **Tests should pin the transaction boundary.** Extract the effectful sequence
  ...
  This follows the [regression test layer](regression-tests-bug-layer.md)
  discipline.
