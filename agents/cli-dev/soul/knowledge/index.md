...
* [lessons/hasownproperty-vs-truthiness.md](lessons/hasownproperty-vs-truthiness.md) - Checking `!obj[key]` to detect an unknown key conflates absent with declared-but-falsy manifest entries; test key existence first, then validate the value with its own error code.
