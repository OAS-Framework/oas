...
* [lessons/cross-instance-writes-commit-last.md](lessons/cross-instance-writes-commit-last.md) - Cross-instance metadata writes in spawn-style multi-step operations must happen only after every remaining fallible step succeeds, or use compensation, so failures do not strand healthy instances pointing at zombies.
