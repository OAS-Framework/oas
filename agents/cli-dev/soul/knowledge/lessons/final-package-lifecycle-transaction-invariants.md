...
description: No-op acquisition must bind installed runtime bytes to the prior deps lock, removal must use the target's own scope map, update must preserve the original catalog selector form, and post-commit convenience writes must be best-effort.
...
4. Convenience metadata maintained after the artifact and lock transaction, such as `ensurePackagesGitignore()`, must be best-effort/non-failing. The authoritative state is the package lock and store, so a post-commit `.gitignore` maintenance failure must not turn a committed transaction into a hard error or inconsistent rollback.
