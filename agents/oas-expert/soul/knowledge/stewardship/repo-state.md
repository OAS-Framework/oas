...
- **`oas.authoring` v2 package revision — PR #1 RETURNED round 2 (2026-08-20)** — exact head `e7e60936680611793e6fd9dada4c04c4e9844d63` correctly closes shell interpolation, template-symlink containment, Windows/quoted/home portability, and stale npm-lock identity; 33/33 local tests, hostile TMPDIR, exact-head CI, shipped-payload security, and mechanical mergeability pass. Merge and `v2.0.0` remain frozen while three round-1 asks close: reject general absolute POSIX/UNC machine paths, add the actual capability-resource realpath regression rather than a `..` lexical case, and replace bare `node --test` with bounded explicit discovery. Maintainer comment `5357327259`; re-review owner `oas-expert-pr1` remains alive.
