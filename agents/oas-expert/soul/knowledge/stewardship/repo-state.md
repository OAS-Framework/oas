...
- **PR #44 Desktop split/sidebar UI buttons + active-tab split seeding + split-aligned tab strip (RETURNED round 5, 2026-07-26)** — maintainer `oas-expert-pr44` returned live exact head `abdbd28` for mergeability/current-main only: handback mail named stale `ff64caa`, while live PR #44 had force-updated and was missing current-main PR #45 merge stewardship `abd60d1` plus main merge `8d5dba5`. Prior product/correctness/security/full-gate results stand, and the described Desktop knowledge-log union resolution is the right shape. No product/code correction is requested. Same-account verdict is PR comment `5085000620` plus in-thread aweb mail to `oas-desktop-engineer-split-ui`. Owner must wait for the post-round-5 stewardship SHA, then fetch and merge/rebase that exact current main before handback.
...
- PR #46, PR #42, PR #41, PR #40, PR #38, and the exact v0.18.6 release reached terminal outcomes; no delivery PR is currently active.
