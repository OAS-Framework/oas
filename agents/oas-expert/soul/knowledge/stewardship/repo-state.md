...
- **`oas.authoring` v2 package revision — PR #1 APPROVED/HOLD round 4 (2026-08-20)** — replacement reviewer correctly returned `9378a6d` because Windows root-relative absolute paths with one leading backslash escaped the UNC-only check; that reviewer RETURN and head movement superseded both round-3 approvals. New exact head `3ff002504f426260524ca1b3e7bc047d49b5857c` matches any leading backslash and pins the exact bytes with `String.fromCharCode(92)`. Maintainer independently proved old acceptance/new rejection; clean install/audit, 35/35, hostile TMPDIR, diff/ref/status, exact-head CI `32381626201`, and CLEAN/MERGEABLE pass. Same-account round-4 approval comment `5357504153`. Replacement fresh-eyes reviewer `reviewer-3ff0025` now APPROVES this unchanged head with no findings and 35/35/diff/security evidence. Merge, immutable `v2.0.0`, and package publication remain frozen only for fresh `oas-expert-1` exact-head re-handback; any RETURN/head movement supersedes current approvals. No catalog/deployment action is authorized.
...
- OAS-Framework/oas-authoring PR #1 round 4 dedicated capability-root v2 release: APPROVED/HOLD 2026-08-20 at exact head `3ff0025`; single-backslash Windows absolute-path fix, all maintainer gates, and fresh-eyes review pass; only fresh architecture/release re-handback remains (see delivery-log).
