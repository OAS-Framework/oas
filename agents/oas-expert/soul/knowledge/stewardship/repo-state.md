...
- **Post-v0.20 official package wave is stalled before delivery** — the v0.20.0 framework contract is published, but all six official repositories remain on their pre-v0.20 main commits and immutable tags; there are no package PRs or new releases, and the framework catalog still points to those old tags. Five leaf `release/capability-materialization-v2` worktrees preserve uncommitted partial revisions (2.0.0/`>=0.20.0` manifests, canonical `configTemplates`, refreshed schemas, portable templates, and the authoring dedicated-root move), but their instances are no longer running and none reached validator/test/consumer-probe, commit, review, or PR. `oas.dev` was correctly held for last and never started. Consequently catalog publication and deployment rebuild/cutover have not begun; existing transitional-v2 deployments must still be rebuilt rather than migrated.
