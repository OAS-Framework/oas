...
- **Capability materialization and config-template synchronization (accepted 2026-07-29)** — founder-approved decision `5519d36` makes package staging transient, materializes actual versioned capability artifacts under gitignored `.agents/capabilities/installed`, renames optional package config sources to `config-templates`/`configTemplates`, preserves one fully locally editable config and adopted base per scope, and requires explicit guided sync/reset/switch behavior. Integrated local head `b189fcb` passed B1/B2/B3 but the one terminal merged-state review RETURNED it for three demonstrated blockers—capability-ID store escape, adopted/backup symlink scope escape, and silent handcrafted-config replacement on first adopt—plus four important scope/rollback/template-root/Git-ignore gaps. `dev-coordinator-capability-materialization` owns one bounded correction, affected+full terminal gates, and exact-head handoff; no PR, publication, global install, or live migration is authorized. Frozen pre-decision init code/docs/audit branches remain donor evidence only.
