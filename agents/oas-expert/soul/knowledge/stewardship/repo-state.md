...
- **`oas.aweb` v2 package revision — PR #1 RETURNED round 1 (2026-08-20)** — exact head `5e33d0eb6afd39066a801744e81f03d25b7df0d5` matches the accepted released-0.20 package direction and passes the clean-clone 56/56 tests, 29/29 released-kernel probes, published-schema byte parity, audit/diff/dry-pack/security, CI, and CLEAN/MERGEABLE refs. Two correctness/release blockers remain: its portability gate accepts demonstrated `/tmp/...`, Windows drive-absolute, and `~/...` machine paths while claiming templates contain no machine-local path; and independent architecture/release co-gate found bare `node --test` can recursively discover stale nested OAS instance-worktree tests, so the 56 count is layout-dependent. Same-account maintainer RETURN is comment `5357599946`; owner `oas-aweb-expert-resume-v2` must add cross-platform path detection/fixtures, explicit paths for all four intended suites, a no-bare-discovery guard, and nested-sentinel proof, then hand back a new exact SHA. Human retired the package-wave coordinator; `oas-expert-1` now coordinates this preserved review directly and receives future exact-head verdicts. Merge and immutable `v2.0.0` remain frozen; no replacement coordinator, tag, catalog, deployment, lifecycle, or notes action is authorized.
- **Post-v0.20 official package and local cutover** — framework v0.20.0 is published; after the human retired the package-wave coordinator, `oas-expert-1` coordinates the preserved package revisions and immutable releases directly for all six official packages (leaf packages first, `oas.dev` last), catalog follow-up, normal global npm/Pi installation, supported local workspace rebuild (no transitional-v2 migration), three-workspace source Desktop restart, and safe branch/worktree cleanup. The five leaf instances are live on unchanged clean launch bases after bounded recovery from Claude trust/onboarding prompt exits; the coordinator checkout is restored, and package work is in progress. `oas.dev` and catalog remain frozen until the five leaf tags are immutable. No credential deletion or old-v2 compatibility subsystem.
