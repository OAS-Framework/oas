...
- **v0.20.0 finalization after PR #66** — fresh `dev-coordinator-capability-finalization` owns the bounded Desktop API v1 range update (`<0.21.0`), v0.20.0 release notes/gates/tag handoff, then dedicated-root/config-template revisions and immutable releases for all six official packages, catalog follow-up, normal global npm/Pi installation, supported local workspace rebuild (no transitional-v2 migration), three-workspace source Desktop restart, and safe branch/worktree cleanup. No raw PR-merge tag, credential deletion, or old-v2 compatibility subsystem.
