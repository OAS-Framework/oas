...
- **OAS_PACKAGE_CATALOG bridge is undocumented in the published kernel (2026-08-25)**: 0.20.1's skills/ and injects/ never mention the env var, and its bundled catalog pins the v1 line — so plain `oas install oas.dev` on 0.20.1 resolves v1, and the activated v2 catalog is reached only via the env bridge until the next kernel release bundles it. Follow-up for the next kernel cut: bundle the v2 catalog (automatic — it is on main) and document the override in the packages skill. Recorded prominently in the clean-room gate's disclosures.
- **aweb host fault refined (2026-08-25)**: local identity founding works (`aw init --username` under any HOME creates a full local .aw root; spawn mints/retires identities against it — proven in the clean room), but HOSTED team join/invite against the oas-framework team fails on this machine (`aw team join` exit 1, `aw team invite` non-JSON). Consequences: the kernel test suite's one environmental failure, and `oas okf harvest` unable to mint new instance identities in this deployment. Existing identities keep working. Needs the hosted-team fault diagnosed (aweb server/cert state), not kernel work.
