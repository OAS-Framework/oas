...
- **`oas.aweb` v2 package revision — PR #1 RETURNED round 1 (2026-08-20)** — exact head `5e33d0eb6afd39066a801744e81f03d25b7df0d5` matches the accepted released-0.20 package direction and passes 56/56 tests, 29/29 released-kernel probes, published-schema byte parity, audit/diff/dry-pack/security, CI, and CLEAN/MERGEABLE refs. Correctness remains blocked because its portability gate accepts demonstrated `/tmp/...`, Windows drive-absolute, and `~/...` machine paths while claiming templates contain no machine-local path. Same-account RETURN is comment `5357599946`; owner `oas-aweb-expert-resume-v2` must add cross-platform path detection and negative fixtures, then hand back a new exact SHA. Merge and immutable `v2.0.0` remain frozen; no tag/catalog/deployment action is authorized.
