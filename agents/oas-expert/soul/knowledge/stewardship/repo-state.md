...
- **Existing-user official-capability migration** — fresh `cli-dev-official-capability-migration` works from PR #57 main. OAS 0.19.0 must preserve 0.18.x v1 locks/stores/config behavior; 0.19.1's catalog enables one guided recursive official migration while preserving config IDs/targets/settings, requiring explicit new trust/requirements, and leaving custom/owned/path capabilities untouched.
