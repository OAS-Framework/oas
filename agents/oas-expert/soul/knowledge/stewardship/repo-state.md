...
- **Post-v0.20 official package wave resumed 2026-08-20** — the v0.20.0 framework contract is published, while all six official repositories still remain on their pre-v0.20 main commits/tags and the catalog still points there. A fresh `dev-coordinator-package-wave` now coordinates five live attached leaf specialists (`oas-{okf,aweb,authoring,jira,linear}-expert-resume-v2`), each sharing and preserving its stopped predecessor's dirty `release/capability-materialization-v2` worktree. Their gate is: finish final-v2 validators/tests/docs/CI and isolated released-0.20 consumer probes, commit, post-commit review, package PR, and exact-head maintainer handoff before any merge/tag. `oas.dev` remains intentionally last after five immutable leaf tags; catalog publication and deployment rebuild/cutover follow, with unsupported transitional-v2 deployments rebuilt rather than migrated.
