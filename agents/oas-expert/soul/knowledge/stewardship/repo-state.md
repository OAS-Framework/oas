...
- **RELEASED v0.21.2 (2026-08-26)** — corrective: package payload content is never a deployment scope. PR #78 merged at exact head `9921145` as `30734a7`; tag `v0.21.2` published both npm packages + Desktop installers; manual bump PR #79 (`6c4d88f`). Team reconciliation and recursive migration now skip any directory under an `oas-package.json` ancestor (manifest-ancestor rule, not path special-casing), so member repos' exported config templates are no longer validated as live scopes; a repo-root scope that ships a payload subdirectory is preserved by construction, and genuine member-scope failures still report. Discovered by the rebuilt flagship deployment's first bare team reconcile; verified there: all six lock rows ok, zero failures, member repos byte-identical.

- **RELEASED v0.21.1 (2026-08-26)** — corrective: artifact provenance and trust are judged scope-exactly. PR #76 merged at exact head `aaf50a5` as `b5a56bf`; tag `v0.21.1`; manual bump PR #77 (`ebae01d`). `doctorPackagesData` and `manifestTrust` now resolve lock rows at the artifact's own scope (the merged-chain lookup falsely flagged legitimate two-scope deployments as `invalid-lock` and could judge trust against a foreign scope's row) — the sibling of the v0.20.1 scope-exact orphan fix, discovered live on the rebuilt deployment. Full consumer map audited (7 sites, 2 fixed, rest already scope-exact); desktop's analogous integrity-only merged lookup in a derived sub-case recorded as follow-up. Verified: doctor clean at all three live scopes on the published artifact.

- **RELEASED v0.21.0 (2026-08-26) — the official catalog is remote.** Founder decision ([decisions/remote-official-catalog.md](../decisions/remote-official-catalog.md)): the catalog's source of truth is `package-catalog.json` on this repo's main, fetched at resolution time from the constant raw.githubusercontent.com URL. PR #74 merged at exact head `f94d196` as `f967e128` after a 3-lens adversarial FAIL round and an A–H fixer round (null-document regression; remote entries validated as untrusted input — https-only urls, safe refs, canonical paths, override files exempt; O_EXCL random-temp 0600 cache; narrow fetch gate; diagnostics-never-fetch ruling with truthful provenance wording; byte-accurate size cap + GitHub-host redirect check; Desktop CLI band widened to `<0.22.0`). Resolution order: `OAS_PACKAGE_CATALOG` override → bounded validated remote fetch (cached) → last-successful cache → bundled seed; locked operations and doctor never fetch; `OAS_CATALOG_FETCH=off` is the air-gap switch. Tag `v0.21.0`; manual bump PR #75 (`a6d1b8e`, after a transient index.lock). Live no-env probes locked the exact released commits from the real GitHub file. Supersedes the OAS_PACKAGE_CATALOG-undocumented follow-up.

- **Clean-room public release gate PASSED (2026-08-25)** — the published v0.20.1 + v2.0.0-catalog line is proven from public artifacts only: npm `@oas-framework/oas@0.20.1` + `@oas-framework/pi@0.20.1` in a hermetic clean room (synthetic HOME, pinned PATH, credential scrub) with the public catalog via `OAS_PACKAGE_CATALOG`. Final record: 13/13 proofs PASS (control + the 12 mandated proofs), 535 assertions 0 failed, 50 self-testing sentinels 0 broken, 75 recorded commands — install/acquire at the exact six tag commits (upstream ls-remote cross-checked), strict v2 lock row shapes, byte-identical bare restore (the kernel even repairs a tampered artifact), profile adoption byte-equal to the released template with diff/sync/reset/adopt round trips, single-flag trust isolation, profile/type targeting, Jira/Linear opt-in absence-then-presence, Pi+Claude scaffold composition plus a real published-pi session, the FULL aweb identity round trip (local `aw init --username` workspace → team-mismatch fail-closed → mint with capabilityMeta → clean retire; hosted `aw team join` remains faulty on this host and is recorded, no longer limiting), no ambient state, generated-store gitignore, exact lifecycle cleanup. Two adversarial audit rounds validated the substance (independent rebuild reproduced every digest bit-for-bit; 5/5 falsification targets bite) and forced harness truthfulness repairs (two tautological sentinels rebuilt as real measurements; disclosure text now names the exact streams without command rows). Clean room preserved at `/private/tmp/oas-package-coordination/cleanroom-p5` with re-runnable harness, records.jsonl, results.json, SUMMARY.md.

- **PR #73 merged 2026-08-25 as `a51d032` — official v2.0.0 catalog activation.** The bundled `package-catalog.json` now pins all six official packages at their immutable v2.0.0 tags with canonical `oas-package` payload roots (okf `62010db1`, aweb `86a31a0f`, jira `64ed25f6`, linear `3c10a366`, authoring `64bcc44b`, dev `ceb99c53`); `oas.review` stays aliased to `oas.dev`; Jira/Linear remain opt-in. Verified against the PUBLISHED 0.20.1 kernel via `OAS_PACKAGE_CATALOG`: `oas install oas.dev` resolves the exact four-package closure at the released commits with all capabilities materialized untrusted, jira/linear acquire individually, and the documented git source spelling is accepted input (normalized only in the lock). Expected-head merge from `94a99243`; required CI green; full kernel gate green in the build worktree. This catalog ships bundled with the NEXT kernel release; until then it is the public catalog consumable via the supported `OAS_PACKAGE_CATALOG` mechanism (the clean-room gate and deployment rebuild consume it that way).

- **RELEASED v0.20.1 (2026-08-25)** — corrective release for the 0.20 capability-materialization line. PR #71 merged at expected exact head `79e834f` as `a9ceeaf`; annotated tag `v0.20.1` (`ded04c3`) drove release run `32888965280` (fully green including all three Desktop installer legs), publishing `@oas-framework/oas@0.20.1`, `@oas-framework/pi@0.20.1`, and GitHub Release v0.20.1. Manual bump PR #72 merged as `a81cae2` (the run pushed `release-bump/v0.20.1` but the org policy still blocked PR creation — this time silently, with an overall-green run). Content: the three approved corrections (scope-exact doctor v2-lock orphan check; own-property config semantics with `__proto__` refusal and underscore-annotation stripping across all four manifest/soul readers; configless `oas use` → typed `E_NO_CONFIG` with a verbatim-executed `oas init --raw` remedy) plus adversarially forced hardening: fail-closed config writes (`unsafe-config-value`/`unsafe-config-key`, incl. U+2028/U+2029/U+0085, mid-value `" #"`, empty values, and the scaffolded `name:` from directory basenames — a documented behavior change), replacer-function conversion of all replacement-string interpolations, and desktop/kernel trust parity (revised-v2 lock reading, the 7 divergent lock shapes refused, v1 digest counting `.git` like the kernel). Built and reviewed by `oas-expert-final-v2-migration` via opus workflows: three adversarial review rounds + final verify, every fix red-first or mutation-proven (final round: 7/7 mutations killed, 47-case lock-parity fuzz clean). Published-artifact verification passed against the npm-installed 0.20.1 (E_NO_CONFIG, `__proto__` refusal, scaffold-name refusal, real path-source acquisition → v2 lock → no false orphan + planted-artifact negative control, full lifecycle spawn/retire probe); the mandated scaffold-only spawnInstance(launch:false) probe passed with clean retirement. Pre-existing follow-ups recorded in PR #71: desktop provenance cross-check vs `.oas-installation.json`, `capabilities/oas-okf` log-append replacement string, `capability:` scaffold write (fails closed today), tilde replacers correct-but-untested, env-dependent aweb-hook test (`aw` on PATH).

- **RELEASED `oas.authoring` v2.0.0 (2026-08-20)** — OAS-Framework/oas-authoring PR #1 merged at expected reviewed head `3ff002504f426260524ca1b3e7bc047d49b5857c` as main commit `64bcc44b642a5b429385a810e6247242040c9686`; annotated tag `v2.0.0` dereferences to that merge and GitHub Release https://github.com/OAS-Framework/oas-authoring/releases/tag/v2.0.0 is published. The package now transports a dedicated self-contained `capabilities/oas-authoring` root for flat capability materialization under released OAS 0.20, while its portable config template remains package-only explicit-adoption source. Four maintainer rounds plus replacement fresh-eyes and architecture/release co-gates closed shell interpolation, realpath/template and capability-boundary proof, cross-platform machine-path completeness including one-leading-backslash Windows absolutes, bounded Node test discovery, and npm-lock identity. Exact-head CI and local/hostile gates passed; the downloaded 29-file checksum asset is byte-identical (`sha256:29a935b3ac1bf193f4291c98d5cc78b22870f08e795bae3b761edc1b7e2872c6`). A fresh full clone at the immutable tag passed 35/35 released-kernel consumer/scaffold tests with a clean tree. Remote feature branch deleted; immutable v1.0.0 untouched; no catalog or local-deployment update.
...
- Framework source and Desktop artifacts are now **0.19.4** (root/pi npm plus
  GitHub Release installers). Official package tags are: oas.okf 1.4.1,
  oas.aweb 1.8.0, oas.authoring/oas.jira/oas.linear/oas.dev 1.0.0; `oas.dev`
  exports oas.review 1.2.0.
...
- **oas.dev v2.0.0 RELEASED (2026-08-25)** — the sixth and final official package is published: OAS-Framework/oas-dev PR #1 merged as `ceb99c53` from expected exact head `2396d9dd` (23 commits), annotated tag `v2.0.0` (tag object `352157c6`) with GitHub release and full 45-file SHA256SUMS.txt; independent fresh-clone verification at the tag passed (byte-identical sums, validator clean, 270/270 tests). Canonical 0.20 shape with the profile byte-identical to v1 (`daf943e7…`), oas.review 2.0.0 lockstep, closure pinned to oas.okf/aweb/authoring v2.0.0, Jira/Linear proven absent-but-installable via the probe's synthetic five-leaf catalog against the published 0.20.0 kernel (45/45 checks). Built and reviewed by `oas-expert-final-v2-migration` under the human's no-agent-spawn directive: opus workflow implementation + three adversarial multi-lens review rounds + final verification, with all fixes red→green proven. `v1.0.0` untouched for 0.19 consumers; remote release branch deleted; local main fast-forwarded. Kernel v0.20.1 is now also released and the official catalog is activated at v2.0.0 (see On main); the clean-room public release proof and the local deployment rebuild are the remaining migration phases.
- **Post-v0.20 official package wave — five leaf packages RELEASED, knowledge custody CLOSED (2026-08-25)** — all five leaf packages are published at immutable v2.0.0 tags: oas.okf (`62010db`), oas.aweb (`86a31a0`), oas.authoring (`64bcc44`), oas.jira (`64ed25f`), oas.linear (`3c10a36`). The three outstanding owner-lineage knowledge harvests merged 2026-08-25 via each package repo's PR #2 after 3-lens review (scope, fresh exact-head strict OKF, semantic truthfulness): oas-jira `fde9297` (head `ff0912f`), oas-aweb `1d692be` (head `77967db`), oas-linear `abb35cf` (fixed head `314dc00` after one truthfulness RETURN — false doctor-template claim and a resurrected superseded test-script shape, corrected by a commissioned fixer since the authoring harvester had retired). Strict OKF re-verified on each merged main; harvest branches deleted. Source specialists for jira/aweb/linear are retired with worktrees/branches removed and mains fast-forwarded; the authoring pair remains (resume-v2 awaits a directly-asked human retirement answer — nonblocking; final-v2 hosts its attached work tree and is deferred with it). Remaining migration, driven by `oas-expert-final-v2-migration` under explicit human authorization: oas.dev v2.0.0, corrective kernel v0.20.1 (doctor v2-lock recognition, own-property/`__proto__`-safe config diagnostics, configless `oas use` remedy), catalog activation pinned to released tags, clean-room public release gate, then rebuild of the local workspace deployment from released artifacts (rebuild, not migration). Catalog and deployment remain untouched until oas.dev v2 and v0.20.1 are immutable.
- Historical open PR #43 remains outside these feature scopes and requires a separate owner decision.

## Recent deliveries

- OAS-Framework/oas-authoring PR #1 dedicated-root v2 release: MERGED 2026-08-20 as `64bcc44` from exact reviewed head `3ff0025`; immutable `v2.0.0` release/checksum asset and fresh tag clone 35/35 probe verified, remote branch deleted, no catalog/deployment change (see delivery-log).
- OAS-Framework/oas-aweb PR #1 round 1 co-gate update: RETURNED remains at `5e33d0e`; independent architecture/release review adds explicit four-suite test paths, no-bare-discovery guard, and nested-worktree sentinel proof to the existing portability fix (see delivery-log).
- OAS-Framework/oas-aweb PR #1 round 1 released-0.20 capability-materialization package revision: RETURNED 2026-08-20 at exact head `5e33d0e`; all direction/security/full gates pass, but cross-platform machine-path portability detection and tests are required (see delivery-log).
- OAS-Framework/oas-authoring PR #1 round 4 dedicated capability-root v2 release: APPROVED/HOLD 2026-08-20 at exact head `3ff0025`; single-backslash Windows fix plus maintainer/fresh-eyes/architecture gates pass; final merge awaits reviewer message-ID evidence and unchanged-head recheck (see delivery-log).
- OAS-Framework/oas-authoring PR #1 round 3 dedicated capability-root v2 release: APPROVED/HOLD 2026-08-20 at exact head `9378a6d`; all maintainer gates and the `oas-expert-1` co-gate pass, merge awaits only the replacement exact-head fresh-eyes verdict (see delivery-log).
- OAS-Framework/oas-authoring PR #1 round 2 dedicated capability-root v2 release: RETURNED 2026-08-20 at exact head `e7e6093`; four fixes and 33/33/hostile-TMPDIR gates pass, three prior portability/test-discovery asks remain (see delivery-log).
...
- **RESOLVED (2026-08-26) — OAS_PACKAGE_CATALOG bridge / bundled-catalog staleness**: superseded by the v0.21.0 remote catalog (founder decision). The published kernel now fetches the catalog from this repo's main at resolution time; the env var is documented as the hermetic override and `OAS_CATALOG_FETCH=off` as the air-gap switch.
- **aweb fault narrowed again (2026-08-26)**: the rebuilt workspace deployment MINTS AND RETIRES hosted-team identities cleanly (live spawn probe: capabilityMeta + per-instance .aw + roster round trip). What still fails is the OLD `~/oas` deployment context: its `.aw` root fails `aw team join` (likely stale/absent workspace.yaml/api-key state there), which blocks `oas okf harvest` for instances homed in `~/oas` (the two remaining oas-expert instances) and explains the kernel suite's one environmental test failure on this host. Existing identities keep messaging. Needs aweb-side state repair in `~/oas/.aw` (or instance re-homing), not kernel work.
- **Desktop merged-map integrity lookup (2026-08-26)**: `agentProviderTrusted` in the desktop reader resolves capability lock rows from a merged closest-wins map; agrees with the kernel on the live two-scope shape, diverges only in a derived locked-but-not-materialized sub-case. Follow-up for the desktop owner (same scope-exact discipline as v0.20.1/v0.21.1).
- **Payload-boundary edge coverage (2026-08-26)**: the v0.21.2 boundary-is-package-root short-circuit is coverage-light, and an `oas-config.yaml` sitting AT a package root beside the manifest is still validated as a scope — both edge shapes noted in PR #78 for a future test-only follow-up.
...
- CI bump-PR step: the ambiguous-refspec failure is **RESOLVED on main** by
  ...
  PR #28, v0.18.4 as PR #31, v0.18.5 as PR #34, v0.18.6 as PR #39, and
  v0.20.1 as PR #72). Needs an org admin to relax the Actions-PR policy to
  fully automate. NOTE (v0.20.1 run `32888965280`): the run can now finish
  with an overall GREEN conclusion while still not creating the PR — the
  `release-bump/vX.Y.Z` branch is pushed but no PR exists. A green release
  run therefore no longer proves the bump landed: always check main's
  package.json version and the open-PR list after a release.
