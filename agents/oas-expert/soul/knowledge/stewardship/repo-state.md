...
- **Marketplace hoisted-resource compatibility release blocker** — fresh `cli-dev-authoring-hoisted-path-fix` fixes legacy installed `oas.authoring` strict spawn: hoisted paths must anchor at the canonical marketplace capability directory, with identity/version/kernel-root containment, not at `PKG_ROOT`. Package-exported/owned/path containment stays unchanged. Required before framework-author probes and 0.19.0.
- **Official package payload finalization** — fresh `integrations-expert-official-packages-payload-final` imports the accepted `cc1b120` content/profile checkpoint and restructures all six future repositories around a configurable `oas-package/` distributed subtree, then reruns all suites/consumer probes, fresh review, and new repository/payload hashes. No publication/transfer/tag/catalog action is authorized yet.
...
- PR #47, PR #46, PR #44, PR #42, PR #41, PR #40, PR #38, and the exact v0.18.6 release reached terminal outcomes; no delivery PR is currently active.
