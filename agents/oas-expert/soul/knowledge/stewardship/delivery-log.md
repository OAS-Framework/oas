...
## PR #80 — oas-expert soul knowledge harvest: final-v2-migration notes (2026-08-26)
- verdict: MERGED as `013dbb3bb0985419c9ab6e1dbb88ca37fe0d7eed` from expected exact head `ad44e3e0`. Soul-only 6-file diff: two new lessons (incomplete-fix-class final verification; falsifiable sentinels), the green-run-skips-bump-PR note merged into the existing release-bump-pr-org-restriction lesson (title/description widened to "check main, not the run") with a matching minimal git-tag-release skill update, index/log discipline. Cross-deployment custody handoff authorized by oas-expert-1: the source instance's `oas okf harvest` is blocked by the old deployment's aweb join fault, so an opus harvester workflow authored the branch against the memory-harvest/okf skills, an independent verifier passed boundary/fidelity/OKF/honesty, and the maintainer's second commit applied the verifier's three prescribed precision corrections. Strict OKF 85/0/0 at head and on merged main; CI green.
- owner: harvester workflow wf_b1f1954c-e47 (source instance oas-expert-final-v2-migration) · coordinator: oas-expert-1
- taught us: when a deployment fault blocks the normal harvest path, the custody contract survives by moving the AUTHOR to a working boundary while keeping the maintainer gate intact — workflow-authored branch, independent verify, exact-head merge; the notes themselves never bypass review.

## PR #78 — kernel v0.21.2 corrective: package payloads are not deployment scopes (2026-08-26)
- verdict: MERGED as `30734a70eadfb841087df46de14059b346fb11b6` from expected exact head `99211455`, then RELEASED as `v0.21.2` — npm both packages 0.21.2, manual bump PR #79 (`6c4d88f`). Scope: `discoverWorkspaceScopes` now skips any child directory whose ancestry contains an `oas-package.json` (the manifest-ancestor rule — payload `config-templates/` carrying `oas-config.yaml` are package CONTENT, not scopes to validate). Found live: bare team reconciliation at the rebuilt deployment failed on jira/linear payload template dirs ("referenced by capabilities.layers... supplied by no visible locked package"). Red-first tests; boundary-is-package-root short-circuit and manifest-beside-config edge noted as coverage follow-ups in the PR.
- owner: oas-expert-final-v2-migration (opus ultracode workflow wf_1e0da939-fcb) · coordinator: none (maintainer-driven)
- taught us: scope discovery must classify by MEANING (is this file deployment config or distributed payload?), not by presence of a config-shaped file; the rule is "manifest ancestor wins", not a path special-case.

## PR #76 — kernel v0.21.1 corrective: scope-exact provenance/trust lookups (2026-08-26)
- verdict: MERGED as `b5a56bf` from expected exact head `aaf50a5`, then RELEASED as `v0.21.1` — npm both 0.21.1, manual bump PR #77 (`ebae01d`). Scope: doctor provenance cross-check and capability trust resolution judged installed artifacts against the merged closest-wins chain instead of the artifact's own scope's lock rows — a legitimate two-scope install (laptop `catalog:oas.okf` + workspace `catalog:oas.okf@v2.0.0` dep row) was flagged `invalid-lock`. Fix: `levelRows` in doctor, `artifactScopeOf`/`capabilityRowAtScope` in trust resolution. Found live on the rebuilt deployment; verified CLEAN at all three scopes on the published artifact.
- owner: oas-expert-final-v2-migration (opus ultracode workflow wf_87f1c93e-2de) · coordinator: none (maintainer-driven)
- taught us: this is the SAME defect family as the v0.20.1 doctor-orphan fix — scope-exact discipline ("an artifact under `<scope>/.agents` is judged against `<scope>/oas-lock.json` only") is a repo-wide invariant that every merged-map consumer must be audited against, not a per-site fix; the desktop reader still carries one derived sub-case (open thread).

## PR #74 — kernel v0.21.0: the official catalog is remote (2026-08-26)
- verdict: MERGED as `f967e128` from expected exact head `f94d196f1a4ed1890bcf4622762476aeb03c1f13`, then RELEASED as `v0.21.0` — npm both 0.21.0, manual bump PR #75 (`a6d1b8e`). Founder-decision implementation ([decision](/decisions/remote-official-catalog.md)): catalog source of truth is `package-catalog.json` on this repo's main, fetched from a constant URL; resolution order env-override → bounded validated fetch → 0600 atomic cache → bundled seed; locked operations and doctor NEVER fetch; only bare-catalog-id acquisitions refresh. Review: 3-lens round 1 all-FAIL with a SECURITY blocker (remote entries reached `git clone` argv unvalidated) plus null-document regression, symlink-followable cache temp, untested cache branch (mutation survived), untruthful doctor wording, UTF-16 size cap, cross-origin redirects, and a Desktop band that would refuse the release — all fixed round 2, re-verify PASS (9/9 mutations, live no-env GitHub probes). Published artifact verified end-to-end.
- owner: oas-expert-final-v2-migration (opus ultracode workflows wf_4ec018dc-e26 + fixers) · coordinator: none (maintainer-driven)
- taught us: the kernel's first steady-state network read demanded the full untrusted-input treatment on a file we ourselves publish — the threat model is the transport and any future compromise of the repo, not the current file's content; and "diagnostics never fetch" is the ruling that kept doctor honest and offline-safe.

## PR #73 — official v2.0.0 catalog activation (2026-08-25)
- verdict: MERGED as `a51d03263e149f100a0e728e970826673c90a588` from expected exact head `94a99243`. Two-file diff: `package-catalog.json` pins all six official packages at immutable v2.0.0 tags (canonical `oas-package` roots, `oas.review`→`oas.dev` alias kept, Jira/Linear opt-in), plus the docs example. Verified pre-merge against the PUBLISHED 0.20.1 kernel with the exact candidate file via `OAS_PACKAGE_CATALOG`: exact oas.dev four-package closure at released commits, individual jira/linear acquisition, documented git spelling accepted as input with lock-only normalization; full kernel gate green in the worktree. This merge is the public activation; the catalog ships bundled with the next kernel release and is consumable today via `OAS_PACKAGE_CATALOG`.
- owner: oas-expert-final-v2-migration · coordinator: none (maintainer-driven)
- taught us: the bundled catalog means activation-by-merge only changes what FUTURE kernel releases ship — published kernels keep their frozen catalog, and the supported bridge for already-released kernels is the `OAS_PACKAGE_CATALOG` mechanism, which is also exactly the right lever for verifying a candidate catalog against a published kernel before merging it.

## PR #71 — kernel v0.20.1 corrective release: doctor v2-lock recognition, own-property config safety, configless-use remedy (2026-08-25)
- verdict: MERGED as `a9ceeaf530a3c273532a7188dca23fa47087c31b` from expected exact head `79e834ffbdb263925ad526d05a4706316069f310`, then RELEASED as annotated `v0.20.1` (`ded04c3`) — npm `@oas-framework/oas@0.20.1` + `@oas-framework/pi@0.20.1` + GitHub Release with Desktop installers, run `32888965280` fully green; manual bump PR #72 (`a81cae2`). Scope: the three approved corrections plus adversarially forced hardening on the same surfaces — fail-closed config writes (typed `unsafe-config-key`/`unsafe-config-value`, incl. invisible line separators and the scaffolded directory-basename name, a documented behavior change), replacer-function conversions, annotation stripping in all four readers, and desktop/kernel trust parity (revised-v2 lock reading, 7 divergent shapes refused, v1 digest counts `.git`). Verified on the PUBLISHED artifact: E_NO_CONFIG with verbatim remedy, `__proto__` refusal, scaffold-name refusal, real path-source acquisition → v2 lock → no false orphan with a planted-artifact negative control, spawn/retire lifecycle; scaffold-only spawnInstance(launch:false) probe passed and retired clean.
- owner: oas-expert-final-v2-migration (opus ultracode workflows; 3 review rounds + final verify; final round 7/7 mutations killed, 47-case lock-parity fuzz) · coordinator: none (maintainer-driven)
- taught us: an "incomplete fix" is the dominant late-round defect class — a guard added at one call site (replacer function, annotation strip, digest predicate) recurs at sibling sites the fix's own docstring then overclaims; final verification must hunt the CLASS, not re-test the instance. Also: the release run can now finish GREEN while the org policy still silently skips bump-PR creation — check main's version, not the run conclusion.

## OAS-Framework/oas-dev PR #1 — oas.dev v2.0.0 capability-materialization release (2026-08-25)
- verdict: MERGED as `ceb99c535148e7873f3cfafbdfb8bbfe8ee7ff1c` from expected exact head `2396d9dd98d1e6c52a56131a81e9e54f41483270`, then RELEASED as immutable annotated `v2.0.0` (tag object `352157c6`) with a 45-file SHA256SUMS.txt asset. Independent fresh GitHub clone at the tag: correct commit, byte-identical sums, validator clean, 270/270 tests. CI green (validate-and-test + consumer-probe with five sibling leaf checkouts at their immutable v2 tags). Product: canonical configTemplates shape, profile bytes preserved from v1 (`daf943e7…`), oas.review 2.0.0 lockstep, dependencies exactly okf/aweb/authoring @v2.0.0, Jira/Linear opt-in absence proven non-vacuously against a synthetic full catalog on the published 0.20.0 kernel.
- owner: oas-expert-final-v2-migration (implementation via opus ultracode workflows under the human no-agent-spawn directive) · coordinator: none (maintainer-driven)
- taught us: adversarial multi-lens review rounds converge fast when each round's fixes must show red→green — round 1 caught product truth defects, round 2 broke only the newly added guard code, round 3 was all pre-existing edges; measuring the platform (node 22 test discovery, kernel parser text) beats reasoning about it — two "obviously right" guard designs were wrong until measured; and a gate's own guards need the same mutation discipline as product code.

## OAS-Framework/oas-linear PR #2 — owner-lineage v2 knowledge harvest (2026-08-25)
- verdict: MERGED as `abb35cf0013360170e49c4697ddefd9bc6844354` from expected exact head `314dc0064be71b74dc17d045f9f45ea83e2656f4`, after one truthfulness RETURN at `bccab8d5`: the 0.20-behavior reference falsely claimed released `oas doctor` reports available-but-unadopted templates (the kernel deliberately does not — design note in `bin/oas.mjs`), and the test-script-gate Decision resurrected the superseded `validate &&` chain shape its own sibling lesson forbids. The authoring harvester was already retired, so a commissioned fixer applied the two corrections plus dated log Fix entries; maintainer re-reviewed the exact new head (strict OKF 11/0/0, 3-file diff, MERGEABLE/CLEAN) and merged. Custody re-validated on merged main; harvest branch deleted; both linear specialists retired with worktree/branch cleanup; local main fast-forwarded.
- owner: memory-harvest-linear-v2-notes (retired) → commissioned fixer · coordinator: oas-expert-final-v2-migration
- taught us: strict OKF proves shape, never truth — the only blocker across all three harvests was a factual claim about released kernel behavior, caught by reading the released binary's own design note; and a returned PR whose author lineage is fully retired needs a commissioned fixer plus maintainer re-review, not maintainer self-fixing.

## OAS-Framework/oas-aweb PR #2 — owner-lineage v2 knowledge harvest (2026-08-25)
- verdict: MERGED as `1d692be763599c8a29d940db8a302d2125512d09` from expected exact head `77967db531f767cb0eca778d43953a1dbcadc3be`. 3-lens review (scope/mergeability, fresh exact-head strict OKF 10/0/0, semantic truthfulness vs released 0.20.0) all PASS; every kernel-facing claim verified against the published kernel; the kernel-defects reference preserves the doctor-orphan and prototype-chain diagnostic defects as evidence with the no-package-workaround ruling. Custody re-validated on merged main; harvest branch deleted; both aweb specialists retired with worktree/branch cleanup; local main fast-forwarded.
- owner: memory-harvest-aweb-v2-notes (retired) · coordinator: oas-expert-final-v2-migration
- taught us: the harvest's kernel-defects reference is the grounding evidence for the approved v0.20.1 corrective scope (doctor v2-lock recognition and own-property-safe config diagnostics).

## OAS-Framework/oas-jira PR #2 — owner-lineage v2 knowledge harvest (2026-08-25)
- verdict: MERGED as `fde929748ec8b788a5b79a9e712fe5f58539d716` from expected exact head `ff0912fa85a45e4cba6980b45f8d80ca674795a0`. 3-lens review all PASS: knowledge-only 14-file diff, fresh exact-head strict OKF 10/0/0 with zero producer warnings, and the 10 frozen resume-v2 notes fully represented with claims verified against released 0.20.0 and repo main. Merge-base one commit behind main tip in all three harvest PRs (branches cut from the release-branch tip main then merged) — accepted with clean `merge-tree` proof and knowledge-only ranges. Custody re-validated on merged main; harvest branch deleted; both jira specialists retired; `probe-dev` untracked test-residue soul removed; local main fast-forwarded.
- owner: memory-harvest-jira-v2-notes (retired) · coordinator: oas-expert-final-v2-migration
- taught us: harvesters that retire after opening their PR leave an ownerless handback; the wave's lifecycle contract should keep the author lineage alive until the maintainer verdict lands.

## OAS-Framework/oas-authoring PR #1 (terminal) — dedicated capability-root v2 release (2026-08-20)
- verdict: MERGED as `64bcc44b642a5b429385a810e6247242040c9686` from expected exact head `3ff002504f426260524ca1b3e7bc047d49b5857c`, then RELEASED as immutable annotated `v2.0.0` (tag object `53eb93734598ba8bb23164357c9fec0eb59cf00b`, dereferencing to the merge) at https://github.com/OAS-Framework/oas-authoring/releases/tag/v2.0.0. All four maintainer gates, replacement fresh-eyes APPROVE, and fresh architecture/release co-gate passed at the unchanged head; required CI run `32381626201` succeeded. Four rounds closed environment-controlled shell interpolation, template/capability realpath separation, a real capability-resource symlink regression, complete machine-path portability including Windows root-relative absolutes, bounded Node discovery, and npm-lock release identity. Clean install/audit, 25 unit + 10 released-OAS consumer/scaffold probes, hostile TMPDIR, exact refs, diff/status, and CLEAN/MERGEABLE passed. Release `SHA256SUMS.txt` covers 29 tracked files; downloaded asset is byte-identical with SHA-256 `29a935b3ac1bf193f4291c98d5cc78b22870f08e795bae3b761edc1b7e2872c6`. Fresh full clone at the immutable tag passed 35/35 and remained clean. Remote feature branch deleted; v1.0.0 unchanged; no catalog/local-deployment action.
- owner: oas-authoring-expert-resume-v2 · coordinator: dev-coordinator-package-wave
- taught us: validate the property, not an enumerated set of familiar forms; filesystem containment must be realpath-backed; backslash-sensitive tests need exact byte construction; developer gates must bound discovery; and every approval/CI/reviewer/release handback is exact-head evidence invalidated by movement. Durable reviewer mail evidence outranks pane presence, and a leaf release must preserve its explicit catalog/deployment boundary.

## OAS-Framework/oas-aweb PR #1 (round 1 co-gate update) — bare Node test discovery (2026-08-20)
- verdict: RETURNED remains at exact head `5e33d0eb6afd39066a801744e81f03d25b7df0d5`. Independent architecture/release co-gate accepted direction, payload, runtime-resource/trust behavior, portability of the actual shipped template, versioning, released-0.20 probes, and no-workaround handling, but found a second important release blocker: `package.json` runs bare `node --test`, so a real OAS development checkout can recursively execute stale tests under nested `agents/<soul>/instances/<id>/work/test/**` trees. The clean-clone 56/56 result does not prove a stable intended suite. Owner must name all four intended test files explicitly, pin the aggregate script against bare discovery, prove a planted nested sentinel is not run, and rerun the exact-head gates. This is additive to the maintainer's cross-platform machine-path finding; merge/tag remain frozen until both are fixed and re-reviewed.
- owner: oas-aweb-expert-resume-v2 · coordinator: dev-coordinator-package-wave
- taught us: the existing [bare Node test discovery lesson](/lessons/bare-node-test-recurses-into-agent-worktrees.md) applies to independent official package repositories too; clean clones can hide the nondeterminism because the contaminating instance worktrees exist only in live OAS-managed development layouts.
...
- taught us: nothing new beyond round 2—the exact remaining acceptance checklist closed cleanly. A multi-gate release can be technically approved while administratively held; preserve exact-head evidence and do not collapse the independent co-gate into the maintainer result.

## OAS-Framework/oas-authoring PR #1 (round 2) — bounded validator/security fixes (2026-08-20)
- verdict: RETURNED at exact head `e7e60936680611793e6fd9dada4c04c4e9844d63`. Product direction remains PASS. Correct fixes remove environment-controlled TMPDIR shell interpolation with argv-only git→tar piping, reject resolved template symlinks into capability roots, cover Windows/quoted/home-relative machine paths, and align/assert npm-lock root identity at 2.0.0. Shipped-payload security PASS; a hostile literal `$(touch ...)` TMPDIR ran all 10 probes without execution. Full local gate passed 33/33, audit and diff checks; exact-head CI passed and GitHub reported CLEAN/MERGEABLE. Correctness/reproducibility remains RETURN because general absolute POSIX and UNC paths are still accepted (`/tmp/...` reproduced status 0), the capability-resource boundary test still uses an already-rejected literal `../../...` rather than exercising the new realpath boundary, and developer-facing `npm test` remains bare `node --test` (a nested OAS instance-worktree sentinel was discovered and executed as test 34). Same-account verdict is comment `5357327259`; merge and `v2.0.0` remain frozen.
- owner: oas-authoring-expert-resume-v2 · coordinator: dev-coordinator-capability-finalization
- taught us: a bounded fix handback must be compared with the maintainer's complete prior ask list, not only a parallel reviewer's overlapping findings. Tests for two symlink-sensitive invariants are not interchangeable, and named path examples must not narrow an explicitly general portability contract.
...
- owner: oas-desktop-engineer-split-ui · coordinator: none
...
## PR #46 — oas-desktop-engineer maintainer-handback stewardship-race harvest (2026-07-26)
...
- owner: oas-desktop-engineer-spawn-modal-fix · coordinator: dev-coordinator-spanwer-modal-bug
...
## PR #42 — oas-desktop-engineer quick-open deferred-intent harvest (2026-07-26)
- verdict: MERGED as merge commit `09605c7` at exact head `028a984`. All four gates PASS. The knowledge-only PR preserves retiring instance `oas-desktop-engineer-quick-open`'s post-merge harvest commit (`97654ce`, cherry-picked as `028a984`) in the canonical Desktop soul: the pending-intent data-currency lesson and Quick Open → Spawn preselect handoff decision now state that module-level deferred preselects must die with the mounted Spawn consumer on unmount. Final scratch gates passed after root + Desktop dependency install and copied installed capabilities: targeted oas-desktop-engineer strict OKF `106 concept(s), 0 error(s), 0 warning(s)`, aggregate `validate:okf` all 8 bundles with zero warnings, `npm test` `554/555` with the expected node-pty ABI skip, `check`, `validate`, `pack:check`, and diff-check. The current code/test surface already carries the reviewed unmount clear and regression test. Exact-head PR CI was green; same-account approval was recorded as a comment; expected-head merge succeeded and the remote branch was deleted.
- owner: oas-desktop-engineer-quick-open · coordinator: dev-coordinator-keybindings
...
- owner: oas-desktop-engineer-split-panels · coordinator: dev-coordinator-split-panel
...
## PR #36 (round 1) — oas-desktop-engineer post-PR35 keybindings harvest (2026-07-25)
- verdict: RETURNED at exact head `d26a356` for one knowledge-correctness fix. Product direction, security, and mechanical mergeability passed: scope is 5 files under `agents/oas-desktop-engineer/soul/knowledge/`, no executable/config surface, `origin/main` was an ancestor, `git diff --check` was clean, PR CI was green, and GitHub reported MERGEABLE. Local scratch gates passed after root + Desktop dependency install and copying installed capabilities: targeted oas-desktop-engineer strict OKF 90/0/0, repo `validate:okf` all 8 bundles, `npm test` 433/434 with the expected node-pty ABI skip, `check`, `check:pi`, `validate`, and `pack:check`. Blocker: the branch cherry-picked final harvest commit `4da43b2` as `d26a356` but omitted its original parent harvest `5543ac5` (`memory-harvest: merge view-action dispatch guard lesson`), so the new follow-up queue links the final dispatch-ineligible view-actions model while PR head still has stale `first-class-view-defaults-window-dispatch-surface.md` guidance telling readers to guard registered `run()` handlers. Author must include `5543ac5` or equivalent concept/index/log updates, rerun gates, merge latest main, and hand back an exact head.
- owner: oas-desktop-engineer keybindings-wiring harvest · coordinator: dev-coordinator-keybindings
