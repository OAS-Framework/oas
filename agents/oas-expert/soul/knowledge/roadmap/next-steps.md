...
1. **Capability registry/npm acquisition**: git/path artifacts are
...
2. **Selector evolution**: V1 groups are explicit soul lists. Consider tags or
   selectors only after real group maintenance pressure, preserving
   deterministic specificity/conflict rules.
3. **Claude session-event parity**: file/skill/instruction composition is
...
4. **First-run diagnosis**: no config means no activated layer. Make empty
   chains and unresolved fundamental layers clearer without silently enabling
   acquired packages.
5. **Control Pane telemetry**: the live TUI covers current lineage, tmux, git,
   task/state, and knowledge surfaces. Define a runtime-neutral adapter event
   contract before adding token/cost, model, tool, capability/trust, or activity
   telemetry; do not inspect pi internals from the universal UI.
6. **Layout adapters**: alternative agents-root structures, after package
