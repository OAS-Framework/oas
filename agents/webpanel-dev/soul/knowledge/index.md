...
* [lessons/workspace-scoped-instance-routing.md](lessons/workspace-scoped-instance-routing.md) - When multiple watched workspaces can contain same-named instances, instance-name APIs must forward ?ws= and findInstance(name, wsId) must fail closed inside that workspace rather than falling back to the first match.
