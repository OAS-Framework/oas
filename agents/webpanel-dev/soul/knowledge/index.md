...
* [lessons/pin-node-test-globs-in-agent-worktrees.md](lessons/pin-node-test-globs-in-agent-worktrees.md) - In OAS agent worktrees, bare `node --test` can recurse into sibling `agents/*/instances/*/work` trees and run stale or destructive copied tests, so npm test must pin explicit repository-owned test globs.
