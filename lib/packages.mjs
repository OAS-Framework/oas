 ...
 */
import { existsSync, readdirSync, readFileSync, realpathSync, renameSync, statSync, mkdirSync, rmSync, writeFileSync } from "node:fs";
...
export function acquirePackage(levelDir, spec) {
  ...
  try {
    for (const entry of closure) {
      const dest = containedPackageDest(levelDir, entry.id);
      const sourceIntegrity = packageIntegrity(entry.dir);
      if (!existsSync(join(dest, OAS_PACKAGE_MANIFEST))) {
        mkdirSync(installedPackagesDir(levelDir), { recursive: true });
        execFileSync("cp", ["-R", entry.dir, dest]);
        staged.push(dest);
      } else {
        // Same-ID destination already present: reuse ONLY an exact-content match.
        // Locking stale installed content against a fresh source manifest would
        // mix metadata from two different trees.
        ...
        if (packageIntegrity(dest) !== sourceIntegrity) {
          fail("integrity-drift", `installed store at ${dest} differs from the source being acquired — OAS never silently updates; remove it or use an explicit upgrade workflow`);
        }
      }
      // ALL lock metadata derives from the MATERIALIZED artifact, not the source.
      const materialized = loadPackageManifest(dest);
      const record = {
        package: entry.id, version: materialized.version,
        // Frozen lock schema: path sources always record commit "local" (a git
        // SHA of a local checkout is provenance display, never lock data).
        commit: "local",
        integrity: packageIntegrity(dest),
        source: `path:${resolve(entry.dir)}`, capabilities: packageCapabilityIds(materialized),
      };
...
/** Validate a lock/package id and derive its store destination, asserting the
 * result is a DIRECT child of the installed root. Lock keys come from
 * checked-in JSON — an id like "." or ".." must never become a path
 * component (a failed restore would then rmSync the whole store). */
function containedPackageDest(level, id) {
  if (!PACKAGE_ID_RE.test(String(id))) fail("invalid-package-manifest", `invalid package id in lock: ${JSON.stringify(String(id))}`);
  const root = installedPackagesDir(level);
  const dest = join(root, packageSlug(id));
  if (dirname(dest) !== root || dest === root) fail("invalid-package-manifest", `package id ${JSON.stringify(String(id))} does not resolve to a direct child of the installed store`);
  return dest;
}
 ...
 * integrity, the lock's capabilities list, and the manifest identity, and
 * install. Never advances a ref. Present artifacts are verified for integrity
 * AND capability-list consistency. NO team-boundary recursion (workstream 2
 * layers that on top).
...
export function restorePackages(startDir, { levels: onlyLevels } = {}) {
  ...
  for (const level of levels) {
    for (const [id, lock] of Object.entries(readPackageLocksAt(level))) {
      let dest;
      try { dest = containedPackageDest(level, id); }
      catch (e) { report.push({ package: id, level, status: "failed", reason: e.message }); continue; }
      if (existsSync(join(dest, OAS_PACKAGE_MANIFEST))) {
        try {
          const integrity = packageIntegrity(dest);
          if (integrity !== lock.integrity) fail("integrity-drift", `installed tree ${integrity} ≠ locked ${lock.integrity}`);
          const m = loadPackageManifest(dest);
          if (m.package !== id) fail("invalid-package-manifest", `installed store provides "${m.package}", lock expects "${id}"`);
          const caps = packageCapabilityIds(m);
          if (JSON.stringify([...caps].sort()) !== JSON.stringify([...(lock.capabilities || [])].sort())) {
            fail("capability-list-mismatch", `lock capabilities [${(lock.capabilities || []).join(", ")}] disagree with installed manifest [${caps.join(", ")}]`);
          }
          report.push({ package: id, level, status: "ok", dir: dest });
        } catch (e) { report.push({ package: id, level, status: "failed", reason: e.message }); }
        continue;
      }
      ...
      // Stage into a unique temp sibling; only the staging dir is ever removed on failure.
      const staging = `${dest}.staging-${process.pid}-${report.length}`;
      try {
        ...
        execFileSync("cp", ["-R", from, staging]);
        const integrity = packageIntegrity(staging);
        ...
        const m = loadPackageManifest(staging);
        if (m.package !== id) fail("invalid-package-manifest", `source provides "${m.package}", lock expects "${id}"`);
        const caps = packageCapabilityIds(m);
        if (JSON.stringify([...caps].sort()) !== JSON.stringify([...(lock.capabilities || [])].sort())) {
 ...
 * A present-but-invalid command (empty, non-string) returns a typed invalid
 * record so the fail-closed policy sees it — only a fully absent command is
 * dropped as "not a requirement". */
export function normalizeRequirement(req) {
  if (!req || typeof req !== "object" || req.command === undefined || req.command === null) return undefined;
