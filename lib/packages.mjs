...
import {
  OAS_LOCK_FILE, listInstalledPackages, loadPackageManifestAt, parseYamlNested,
  readPackageLocks, resolveOasConfig, validateConfigShape,
} from "./core.mjs";
...
export const RUNTIME_PACKAGE_MANAGERS = {
  pi: {
    ...
    /** Installed sources from pi's settings — entries are a source string or { source }. */
    installed: (env = process.env) => {
      const file = join(env.HOME || "", ".pi", "agent", "settings.json");
...
export function normalizeRequirement(req) {
  ...
  if ("runtime" in req && req.runtime !== undefined) {
    ...
    return {
      ...
      command: `${runtime}:${packageSpecIdentity(pkg)}`,     // the requirement's identity/consent key
      ...
      _invalid: !RUNTIME_PACKAGE_MANAGERS[runtime]
        ...
        : !safeRuntimePackageSpec(pkg)
          ? `runtime package spec is not a plain source token: ${JSON.stringify(pkg)}`
          : undefined,
    };
  }
...
export function requirementInstallPlan(req, { platform = process.platform } = {}) {
  ...
  if (r.kind === "runtime-package") {
    ...
    return {
      ...
      manager: r.runtime, argv: mgr.argv(r.package), source: `${r.runtime} package (${r.package})`,
      scope: mgr.scope, runtime: r.runtime, package: r.package,
      version: (String(r.package).slice(packageSpecIdentity(r.package).length).match(/^@(.+)$/) || [])[1],
    };
  }
