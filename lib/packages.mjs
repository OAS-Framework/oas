...
import {
  ...
  readlinkSync, realpathSync, rmdirSync, rmSync, statSync,
} from "node:fs";
...
/** Fail closed when an INTERMEDIATE component of a protected path is a symlink
 * leaving the scope: restoring through it would delete or rewrite outer-scope
 * state that this run never owned. A symlink at the protected path itself is
 * fine — it is captured and restored verbatim, never written through. */
function assertJournalContainment(scopeReal, rel) {
  ...
  for (const part of parts.slice(0, -1)) {
    cur = join(cur, part);
    let st;
    try { st = lstatSync(cur); } catch { return; } // absent from here down: nothing to escape through
    if (st.isSymbolicLink() && !isInside(scopeReal, realpathSync(cur))) {
...
function copyExact(from, to) {
  ...
  cpSync(from, to, { recursive: true, dereference: false, verbatimSymlinks: true, preserveTimestamps: true });
}
...
export function beginRunJournal(scopeDir, { backupRoot = tmpdir(), extraPaths = [] } = {}) {
  ...
  const rels = [...RUN_JOURNAL_PATHS, ...extraPaths];
  for (const rel of rels) {
    if (isAbsolute(rel) || rel.split(/[\\/]/).includes("..")) fail("E_JOURNAL_PATH_ESCAPE", `journalled path must stay inside the scope: ${rel}`);
  }

  const backupDir = mkdtempSync(join(backupRoot, "oas-run-journal-"));
  // The backup must not live inside the tree it protects: restoring a directory
  // means deleting it first, which would delete the backup with it.
  ...
  for (const rel of rels) {
    ...
    const backup = state.kind === "absent" ? null : join(backupDir, rel.replace(/[\\/]/g, "__"));
    if (backup) copyExact(path, backup);
    entries.push({ rel, path, backup, ...state });
  }

  const anchorPath = join(scope, AGENTS_ANCHOR);
  const anchorExisted = preexistingDirs.has(anchorPath) || existsSync(anchorPath);
