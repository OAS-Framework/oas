...
import {
  OAS_LOCK_FILE, configChain, parseYamlNested, resolveOasConfig, validateConfigShape,
} from "./core.mjs";
...
/** Directory slug of a package id inside the installed store ("example.engineering" → "example-engineering"). */
export const packageSlug = (id) => String(id).replace(/[^a-zA-Z0-9]+/g, "-").replace(/^-|-$/g, "").toLowerCase();
...
function packageRelativePath(pkgDir, rel, what, pkgId) {
  if (typeof rel !== "string" || !rel.trim()) throw new Error(`package ${pkgId}: ${what} must be a non-empty package-relative path`);
  if (isAbsolute(rel) || rel.split(/[\\/]/).includes("..")) throw new Error(`package ${pkgId}: ${what} must stay inside the package: ${rel}`);
  const abs = join(pkgDir, rel);
  if (existsSync(abs)) {
    const fromRoot = relative(realpathSync(pkgDir), realpathSync(abs));
    if (fromRoot === ".." || fromRoot.startsWith(`..${sep}`) || isAbsolute(fromRoot)) {
      throw new Error(`package ${pkgId}: ${what} escapes the package after symlink resolution: ${rel}`);
    }
  }
  return abs;
}
...
export function loadPackageManifest(dir) {
  ...
  catch (e) { throw new Error(`invalid package manifest JSON ${file}: ${e.message}`); }
  const id = m.package;
  if (!id || typeof id !== "string") throw new Error(`package manifest needs "package": ${file}`);
  if (!PACKAGE_ID_RE.test(id)) throw new Error(`invalid package id "${id}" in ${file}`);
  if (!m.version || !m.description) throw new Error(`package ${id} manifest needs version and description (${file})`);
  if (m.capabilities !== undefined && (!Array.isArray(m.capabilities) || m.capabilities.some((c) => typeof c !== "string"))) {
    throw new Error(`package ${id} "capabilities" must be an array of package-relative directories (${file})`);
  }
  ...
  if (m.configs !== undefined && (typeof m.configs !== "object" || Array.isArray(m.configs) || m.configs === null)) {
    throw new Error(`package ${id} "configs" must be a map of profile name → { path, description?, default? } (${file})`);
  }
  const defaults = [];
  for (const [name, spec] of Object.entries(m.configs || {})) {
    if (!PROFILE_NAME_RE.test(name)) throw new Error(`package ${id} config profile name "${name}" must be lowercase alphanumeric/hyphens (${file})`);
    if (!spec || typeof spec !== "object" || typeof spec.path !== "string") throw new Error(`package ${id} config profile "${name}" needs "path" (${file})`);
    packageRelativePath(dir, spec.path, `config profile "${name}" path`, id);
    if (spec.default) defaults.push(name);
  }
  if (defaults.length > 1) throw new Error(`package ${id} marks multiple default config profiles (${defaults.join(", ")}) — at most one may be default (${file})`);
  if (m.dependencies !== undefined && (!Array.isArray(m.dependencies) || m.dependencies.some((d) => typeof d !== "string"))) {
    throw new Error(`package ${id} "dependencies" must be an array of package source specifications (${file})`);
  }
...
export function packageCapabilityIds(manifest) {
  ...
  for (const rel of manifest.capabilities || []) {
    ...
    if (!existsSync(mf)) throw new Error(`package ${manifest.package}: capability path "${rel}" has no oas.json`);
    const cap = JSON.parse(readFileSync(mf, "utf8"));
    if (!cap.capability) throw new Error(`package ${manifest.package}: ${rel}/oas.json has no "capability"`);
...
export function packageCapabilityManifest(manifest, capabilityId) {
  for (const rel of manifest.capabilities || []) {
    ...
    const cap = JSON.parse(readFileSync(mf, "utf8"));
    if (cap.capability === capabilityId) return { ...cap, _dir: join(manifest._dir, rel) };
  }
  return undefined;
}
...
export function readPackageLocksAt(levelDir) {
  ...
  const out = {};
  for (const [id, lock] of Object.entries(parsed.packages || {})) out[id] = { ...lock, _file: file, _level: levelDir };
  return out;
}

/** All visible package locks from startDir upward (closest scope wins per package id). */
export function readPackageLocks(startDir) {
  const out = {};
  for (const cfg of [...configChain(startDir)].reverse()) Object.assign(out, readPackageLocksAt(cfg._level));
  return out;
}
...
export function lockedPackageCapabilities(startDir) {
  ...
  for (const [pkgId, lock] of Object.entries(readPackageLocks(startDir))) {
...
export function resolvePackageSource(src, dir, { clone } = {}) {
  ...
  // Installed/locked package id visible from dir.
  const lock = readPackageLocks(dir)[src];
  if (!lock) throw new Error(`"${src}" is not a locked package id, local path, or git URL at ${dir} — acquire it first with \`oas install <source>\``);
 ...
 * Multiple unmarked profiles require an explicit choice. */
export function selectProfile(manifest, name) {
  const configs = manifest.configs || {};
  const names = Object.keys(configs);
  if (!names.length) throw new Error(`package ${manifest.package} exports no config profiles`);
  if (name) {
    if (!configs[name]) throw new Error(`package ${manifest.package} has no config profile "${name}" (profiles: ${names.join(", ")})`);
  ...
  throw new Error(`package ${manifest.package} has multiple config profiles and none marked default (${names.join(", ")}) — pass --config <name>`);
}
  ...
  for (const scope of scopes) {
    ...
    for (const cap of resolved.capabilities || []) {
      for (const raw of cap.manifest?.requires || []) {
        ...
        if (!r || commandOnPath(r.command, env)) continue;
        if (!byCommand.has(r.command)) {
          byCommand.set(r.command, { command: r.command, why: r.why, docs: r.install.docs, plan: requirementInstallPlan(raw, { platform }), requestedBy: [] });
        }
