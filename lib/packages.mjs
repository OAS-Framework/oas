...
export function readPackageLocks(startDir) {
  ...
  for (const cfg of [...configChain(startDir)].reverse()) {
    ...
    if (lock && Object.keys(lock.capabilities).length) legacy.push({ file: lock.file, level: lock.level, capabilities: lock.capabilities });
  }
...
export function resolvePackageSource(src, dir, { clone } = {}) {
  ...
  const lock = readPackageLocks(dir).packages[src];
