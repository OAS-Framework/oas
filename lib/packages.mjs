...
import {
  ...
  runtimePackageInstalled, safeRuntimePackageSpec, validateConfigShape,
} from "./core.mjs";
...
export { packageSpecIdentity, RUNTIME_PACKAGE_MANAGERS, runtimePackageInstalled, safeRuntimePackageSpec };
export { runtimePackageExtensions } from "./core.mjs";
    ...
    for (const cap of active.values()) {
      for (const raw of cap.manifest?.requires || []) {
        ...
        if (r.kind === "runtime-package") {
          ...
          if (!targets.runtimes.has(r.runtime)) continue;
