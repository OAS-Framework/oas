import { collectControlPane, capturePreview, relativeAge, switchToInstance, openInSplit, workspaceName } from "./model.mjs";
...
function applyTheme(name) {
  ...
  if (name === "solarized") {
    // Solarized Light (Ethan Schoonover) — base3/base2 surfaces; emphasized
    // content = base01, body = base00, secondary = base1 (per the spec's
    // light-mode role assignments — darker means more emphasis on light bg).
    Object.assign(C, {
      ink: fg(88, 110, 117), muted: fg(101, 123, 131), faint: fg(147, 161, 161),
      cyan: fg(38, 139, 210), violet: fg(108, 113, 196), green: fg(133, 153, 0),
      amber: fg(181, 137, 0), red: fg(220, 50, 47),
      guide: fg(131, 148, 150),
      // Flat surface: cards sit on the panel; only the SELECTED card gets a
      // subtle base2 highlight. No full-width color blocks.
      panel: bgc(253, 246, 227), card: bgc(253, 246, 227), selected: bgc(238, 232, 213),
      bar: bgc(238, 232, 213),
    ...
    Object.assign(C, {
      ...
      guide: fg(86, 98, 140),
      // Flat surface: cards sit on the panel; only the SELECTED card gets a
      // subtle highlight. No full-width color blocks.
      panel: bgc(18, 20, 30), card: bgc(18, 20, 30), selected: bgc(30, 34, 50),
      bar: bgc(13, 15, 23),
      bold: `${ESC}1m`, dim: `${ESC}2m`,
    });
    SOUL_PALETTE = [
...
// Every rendered row passes through line(): it owns the row's background and
// default ink, and it NEUTRALIZES terminal-palette bleed — any SGR that
// resets to the terminal's defaults (full reset, incl. compound "0;…" and the
// short \x1b[m form, default-fg 39, default-bg 49) is re-anchored to the theme
// so the user's terminal colors never paint pane surfaces.
function anchorSgr(text, bg) {
  return String(text).replace(/\x1b\[([0-9;:]*)m/g, (seq, params) => {
    // Walk params in order tracking whether fg/bg END the sequence at the
    // terminal default; only those get re-anchored (e.g. "0;31" keeps red).
    const parts = params === "" ? ["0"] : params.split(";");
    let defFg = false, defBg = false;
    for (let i = 0; i < parts.length; i++) {
      const p = parts[i] || "0";
      if (p === "0") { defFg = true; defBg = true; }
      else if (p === "39") defFg = true;
      else if (p === "49") defBg = true;
      else if (p === "38" || p === "48") { // extended color consumes args
        const isFg = p === "38";
        if (isFg) defFg = false; else defBg = false;
        i += parts[i + 1] === "2" ? 4 : parts[i + 1] === "5" ? 2 : 0;
      }
      else if (/^3[0-7]$/.test(p) || /^9[0-7]$/.test(p)) defFg = false;
      else if (/^4[0-7]$/.test(p) || /^10[0-7]$/.test(p)) defBg = false;
    }
    if (!defFg && !defBg) return seq;
    return seq + (defBg ? bg : "") + (defFg ? C.ink : "");
  });
}
function line(width, content = "", bg = C.panel) {
  const visible = clean(content);
  const body = visible.length <= width ? content + " ".repeat(width - visible.length) : clip(visible, width);
  return bg + C.ink + anchorSgr(body, bg) + RESET;
}
...
function branchChip(git, active) {
  ...
  const label = feature
    ? paint(git.branch, C.violet, C.bold)
...
function zoomFrame(instance, preview, width, height) {
  ...
  output.push(line(width, `  ${paint("space/esc", C.ink)} back   ${paint("enter", C.ink)} attach   ${paint("s/S", C.ink)} split   ${paint("q", C.ink)} quit`, C.bar));
...
export function renderFrame(snapshot, state, columns, rows) {
  ...
  const message = state.message
    ...
    : [
        `${paint("↑↓", C.ink)} move`, `${paint("space", C.ink)} zoom`, `${paint("enter", C.ink)} attach`, `${paint("s/S", C.ink)} split`,
...
export async function startControlPane(root, opts = {}) {
  ...
  const onData = (data) => {
    ...
    } else if (input.type === "s" || input.type === "S") {
      // Split the tmux client: pane stays here, the instance's live window
      // opens beside (s) or below (S) via a grouped session.
      const inst = selectedInstance();
      if (!process.env.TMUX) { state.message = "Splits need the pane to run inside tmux (try: tmux attach -t pi-agents)"; draw(); }
      else if (!openInSplit(inst, input.type === "S" ? "v" : "h")) { state.message = "Instance is idle — nothing to open in a split"; draw(); }
      else { state.message = `Opened ${inst.instance} in a split — close it with C-d or your tmux prefix`; draw(); }
