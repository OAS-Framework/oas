...
// ── Theme ── the pane infers the terminal's theme (OSC 11 background query,
// COLORFGBG fallback) and picks a light or dark palette; both hold readable
// contrast on their surfaces. C is mutable: applyTheme() fills it at startup.
const C = {};
let SOUL_PALETTE = [];
let BADGE_TEXT = "";
...
function applyTheme(light) {
  ...
  if (light) {
    Object.assign(C, {
      ink: fg(31, 35, 40), muted: fg(87, 96, 106), faint: fg(140, 149, 159),
      cyan: fg(9, 105, 218), violet: fg(130, 80, 223), green: fg(26, 127, 55),
      amber: fg(154, 103, 0), red: fg(207, 34, 46),
      guide: fg(140, 149, 175),
      panel: bgc(246, 248, 250), card: bgc(255, 255, 255), selected: bgc(221, 231, 245),
      bar: bgc(234, 238, 242),
      bold: `${ESC}1m`, dim: `${ESC}2m`,
    });
    SOUL_PALETTE = [
      { bg: bgc(9, 105, 218), fg: fg(9, 105, 218) },      // blue
      { bg: bgc(130, 80, 223), fg: fg(130, 80, 223) },    // violet
      { bg: bgc(26, 127, 55), fg: fg(26, 127, 55) },      // green
      { bg: bgc(154, 103, 0), fg: fg(154, 103, 0) },      // amber
      { bg: bgc(191, 57, 137), fg: fg(191, 57, 137) },    // pink
      { bg: bgc(32, 125, 145), fg: fg(32, 125, 145) },    // teal
    ];
    BADGE_TEXT = fg(255, 255, 255);
    C.featureBranchBg = bgc(230, 220, 250); C.featureBranchFg = fg(90, 50, 160);
  } else {
    Object.assign(C, {
...
applyTheme(false); // default dark until detection runs

/** OSC 11: ask the terminal its background color; resolve true when light.
 * Falls back to COLORFGBG ("fg;bg" — bg 7/15 = light), then dark. */
export function parseOsc11(response) {
  const m = String(response).match(/\]11;rgb:([0-9a-fA-F]{2,4})\/([0-9a-fA-F]{2,4})\/([0-9a-fA-F]{2,4})/);
  if (!m) return undefined;
  const chan = (hex) => parseInt(hex.slice(0, 2), 16) / 255; // 16-bit gives 4 digits; the top byte suffices
  const [r, g, b] = [chan(m[1]), chan(m[2]), chan(m[3])];
  return 0.2126 * r + 0.7152 * g + 0.0722 * b > 0.5;
}
function detectLightTerminal(timeoutMs = 150) {
  return new Promise((done) => {
    ...
    const finish = (light) => {
      ...
      if (light === undefined) fallback(); else done(light);
    };
    const onData = (d) => {
      buf += d.toString("latin1");
      const light = parseOsc11(buf);
      if (light !== undefined) finish(light);
    };
...
function buildCard(row, ctx) {
  ...
  const errored = instance.sessionTail?.state === "error";
  const errMark = errored ? `  ${paint("✗ err", C.red, C.bold)}` : "";
  const branch = branchChip(instance.git, active);
  let left = `${rail}${tree}${marker} ${soul}  ${name}${errMark}  ${meta}`;
  ...
  if (spare < 1) {
    ...
    left = `${rail}${tree}${marker} ${soul}  ${paint(clip(instanceLabel(instance), room), on ? C.ink : C.muted, C.bold)}${errMark}  ${meta}`;
  }
  ...
  if (expanded && contentLines > 0) {
    ...
    const tailError = errored ? clean(instance.sessionTail?.errorMessage || "session error") : "";
    if (previewMode && on) {
      ...
      if (tailError) lines.push(line(width, `${rail}${spine}${paint("✗ " + clip("session error: " + tailError, innerWidth - 2), C.red, C.bold)}`, bg));
      ...
      if (tailError) field("error", tailError, C.red, 3);
...
function zoomFrame(instance, preview, width, height) {
  ...
  if (instance.sessionTail?.state === "error") {
    output.push(line(width, `  ${paint("✗ session error: " + clip(instance.sessionTail.errorMessage || "", width - 22), C.red, C.bold)}`, C.panel));
  }
  const body = height - 2 - output.length;
  const captured = capturedSgr(preview).split("\n").slice(-body);
