  ...
  for (const line of lines) {
    ...
    if (kind === "pi") {
      ...
      if (entry.message.stopReason === "error") {
        result = { state: "error", errorMessage: String(entry.message.errorMessage || "session error").trim().slice(0, 500), ts };
      ...
      if (entry.isApiErrorMessage || entry.error) {
        ...
        result = { state: "error", errorMessage: String(raw).trim().slice(0, 500), ts };
      } else result = { state: "ok", errorMessage: null, ts };
    }
  }
  return result;
...
export function sessionTailState(instance) {
  ...
  if (!text) return { state: "unknown", errorMessage: null, ts: null };
  const lines = text.split("\n").filter(Boolean);
  if (lines.length > 1) lines.shift(); // first tail line may be a truncated record
