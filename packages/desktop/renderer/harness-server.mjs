...
const server = createServer((req, res) => {
  ...
  if (url.pathname.startsWith("/api/")) {
    // proxy to the oas-web server, preserving method/body; rewrite Host so
    // the oas-web loopback guard sees a loopback host.
    const p = httpRequest({
      hostname: api.hostname, port: api.port, path: url.pathname + url.search,
      method: req.method,
      headers: { ...req.headers, host: `${api.hostname}:${api.port}`, origin: `http://${api.hostname}:${api.port}` },
