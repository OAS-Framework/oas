...
test("acceptProbe: exact v1 payload accepted; every deviation rejected with a reason", () => {
  ...
  assert.equal(acceptProbe(PROBE("0.21.0")).ok, true, "the 0.21 kernel line is inside the band");
  assert.equal(acceptProbe(PROBE("0.21.9")).ok, true);
  const cases = [
    [null, /no probe/],
...
test("band edges: released 0.21.x is accepted, 0.22.0 and the pre-0.18 floor are not", () => {
  assert.deepEqual(ACCEPT_RANGE, { min: [0, 18, 0], maxExclusive: [0, 22, 0] });
  assert.equal(ACCEPT_RANGE_TEXT, ">=0.18.0 <0.22.0");
