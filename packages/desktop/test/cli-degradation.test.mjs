...
const payload = (ok, extra = {}) => ({
  ...
  source: ok ? "path" : null, required: { desktopApi: 1, range: ">=0.18.0 <0.22.0" },
  install: "npm install -g @oas-framework/oas@0.19.0",
  probedAt: 1, tried: ok ? [] : [{ path: "/old/oas", source: "path", reason: "version 0.17.6 outside >=0.18.0 <0.22.0", version: "0.17.6" }],
  ...extra,
});
const jsonCtx = (state) => ({
...
test("cliCard renders the full contract surface: detected, required, Choose, Retry, docs, copyable install", async () => {
  ...
  // required range + api
  assert.ok(el.textContent.includes(">=0.18.0 <0.22.0"), "required range shown");
...
test("cli-status: install/requirement helpers prefer the backend payload, fall back version-lessly", () => {
  const withPayload = { required: { desktopApi: 1, range: ">=0.18.0 <0.22.0" }, install: "npm install -g @oas-framework/oas@0.19.0" };
  assert.equal(cs.cliInstallCommand(withPayload), "npm install -g @oas-framework/oas@0.19.0");
  assert.equal(cs.cliRequirementText(withPayload), ">=0.18.0 <0.22.0 with desktop API 1");
