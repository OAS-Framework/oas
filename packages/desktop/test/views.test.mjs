...
test("Instances roster: anonymous clusters keep accessible group semantics (review 2df15ae)", async (t) => {
  const { JSDOM } = await import("jsdom");
  ...
  const inst = await import("../renderer/views/instances.mjs");
  const roster = [
    { instance: "coord-1", running: true, runtime: "pi", repoName: "r", task: "" },
    { instance: "dev-a", parentInstance: "coord-1", running: false, runtime: "pi", repoName: "r", task: "" },
    { instance: "loner", running: true, runtime: "pi", repoName: "r", task: "" },
  ];
  const ctx = {
    api: (pathname) => Promise.resolve({ ok: true, status: 200, json: async () => pathname.startsWith("/api/panel")
      ? { instances: roster, workspace: { id: "w" }, workspaces: [] }
      : { turns: [], available: false } }),
    openTerminal: () => {},
  };
  try {
    inst.mount(dom.window.document.getElementById("host"), ctx);
    await new Promise((r) => setTimeout(r, 0)); await new Promise((r) => setTimeout(r, 0));
    const doc = dom.window.document;
    const groups = [...doc.querySelectorAll('[role="group"]')];
    assert.equal(groups.length, 1, "one labelled group for the one multi-member cluster");
    const label = groups[0].getAttribute("aria-label");
    assert.match(label, /2 related instances/, "group label carries the member count");
    assert.match(label, /1 running/, "group label carries the running count");
    assert.ok(!label.includes("coord-1"), "no derived cluster name leaks into the accessible label");
    const head = groups[0].querySelector(".ghead");
    assert.equal(head.getAttribute("aria-hidden"), "true", "visual count-only header is decorative");
    assert.ok(!head.textContent.includes("coord-1"), "no visible derived name");
    // the singleton renders without any group/header chrome
    assert.ok([...doc.querySelectorAll(".inst")].some((el) => el.textContent.includes("loner")));
  } finally {
    inst.unmount();
