...
//   {"schemaVersion":1,"name":"@oas-framework/oas","version":"0.21.x","desktopApi":1}
// Desktop accepts desktopApi === 1 and semver >=0.18.0 <0.22.0 (the band is
...
// Widened to <0.22.0 for the v0.21.0 release (the remote official catalog):
// that change is entirely inside package RESOLUTION — the Desktop v1 surface is
// UNCHANGED across the kernel bump (`version --json`, `spawn --json`,
// `okf harvest --json` — test/cli-json-contract.test.mjs), so DESKTOP_API stays
// 1 and only the upper bound moves.
export const ACCEPT_RANGE = { min: [0, 18, 0], maxExclusive: [0, 22, 0] };
