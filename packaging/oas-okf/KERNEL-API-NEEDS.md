...
The package is prepared against the accepted structured CLI design while WS1 finalizes the corrected addendum. Capability command dispatch supplies the canonical absolute CLI path as `OAS_CLI_BIN`; harvest invokes that path with `execFile` and never searches `PATH`:
...
The prepared package no longer discovers the kernel root, calls `oas root`, searches `PATH`, or imports `lib/core.mjs`. Binary selection is isolated in `packageRuntimeCli()`, which requires an absolute `OAS_CLI_BIN`; execution uses argv-safe `execFile`. Compatibility floor `>=0.19.0` and the released consumer fixture will version the corrected boundary once broadcast.
