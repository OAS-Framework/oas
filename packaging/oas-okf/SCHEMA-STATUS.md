...
- `TODO(engine-freeze)`: replace the inherited `compatibility.oas` value (`>=0.6.2`) in both manifests with the OAS release that ships lockfile v2 and the supported package-runtime boundary.
- `TODO(package-runtime-boundary)`: replace the private `lib/core.mjs` import described in `KERNEL-API-NEEDS.md` before release.
