...
const packageSchemaPath = join(root, "schemas", "oas-package.schema.json");
const capabilitySchemaPath = join(root, "schemas", "capability-manifest.schema.json");
...
for (const [index, capabilityDir] of declaredCapabilities.entries()) {
  ...
  for (const [name, command] of Object.entries(manifest.commands || {})) safeResource(capabilityRoot, command.trim().split(/\s+/)[0], `${capabilityDir}/oas.json.commands.${name}`, "command entrypoint");
  for (const [event, command] of Object.entries(manifest.hooks || {})) safeResource(capabilityRoot, command.trim().split(/\s+/)[0], `${capabilityDir}/oas.json.hooks.${event}`, "hook entrypoint");
