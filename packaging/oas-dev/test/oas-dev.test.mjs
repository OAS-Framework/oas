...
test("reviewer operating loop requires both packaged review skills", () => {
  ...
  assert.match(instructions, /aw mail send --to <parentInstance>/);
...
test("developer injection preserves paired harvest and reviewer discipline", () => {
  ...
  assert.match(injection, /After every substantive commit, launch BOTH service agents/);
  assert.match(injection, /oas okf harvest/);
